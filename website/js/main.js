    ...
    dataReady = function() {//This is called as soon as data is loaded
		...
		a.iterNodes(
			function (b) { //This is where we populate the array used for the group select box
				...
				// alert(b.x);
				a.clusters[b.attr.attributes['Group']] || (a.clusters[b.attr.attributes['Group']] = []);
				a.clusters[b.attr.attributes['Group']].push(b.id);//SAH: push id not label
				a.color_family[b.attr.attributes['Group']] = b.color;
			}
		);
...
function configSigmaElements(config) {
    ...
    var a = [],
        b,x=1;
		for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + sigInst.color_family[b] + ';display:inline-block"></div> ' + b + ' (' + sigInst.clusters[b].length + ' members)</a></div>');
    //a.sort();
    ...
    if (f.attributes) {
        ...
        for (var attr in f.attributes) {
			...
			if (attr!=image_attribute) {
				if (attr=='Intezer Analyze Report')
				{
					h = '<span><strong>Intezer Analyze Report:&nbsp;  <a target="_blank" rel="noopener noreferrer" href="' + d + '">View</a></strong></span><br/><br/><br/>';
					e.push(h)
				}
				else
				{
					if (attr != 'Architecture' && attr != 'Gene Count') 
					{
        ...
        if (image_attribute) {
        	...
        	$GP.info_name.html("<div><span onmouseover=\"sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex['" + b.id + '\'])" onmouseout="sigInst.refresh()"><h3>' + b.label + "</h3></span></div><br/>");
        }
